...
// 🏠 MAIN COMPONENT
// ════════════════════════════════════════════
export default function Installations({ installations: propInstallations, stats: propStats, isLoaded = false, onRefresh, isSupervisor }) {
    ...
    return (
        <div className="space-y-4 pb-20 min-h-screen">
            ...
            {/* Modal */}
            <BatchDetailModal key={selectedBatch?.batchKey || selectedBatch?.id || "installations-empty"} batch={selectedBatch} isOpen={modalOpen} onClose={closeModal} onSave={handleSave} saving={saving} notify={notify} isSupervisor={isSupervisor} />
