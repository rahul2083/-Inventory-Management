    ...
    const handleSavePayment = async (e) => {
        ...
        try {
            await axios.post(`${API_BASE_URL}/api/orders/batch-payment`, {
                ...
                itemIds: paymentBatch.map(item => item.id),
                paymentDate: paymentForm.paymentDate,
                totalAmount: paymentForm.amount,
                utrId: paymentForm.utrId,
                status: "Completed"
            }, { headers: { Authorization: `Bearer ${JSON.parse(localStorage.getItem("pt_user"))?.token}` } });
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {currentDispatches.length === 0 ? (
                                ...
                                currentDispatches.map((group, index) => {
                                    ...
                                    return (
                                        <tr key={index} className={`transition-colors ${activeTab === 'payment' ? 'hover:bg-emerald-50/30' : isGroupDue ? 'bg-yellow-50/80 hover:bg-yellow-100/80 border-l-4 border-yellow-400' : 'hover:bg-indigo-50/30'}`}>
                                        ...
                                        <div className="mb-8">
                                            ...
                                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-100">
                                                <div>
                                                    ...
                                                    <p className="text-sm font-bold font-mono text-slate-800 break-all">{details.customerName || details.customer}</p>
                                                </div>
                                                <div>
