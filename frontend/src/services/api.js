    ...
    changePassword: async (data) => {
        ...
        getReturns: async () => {
            try {
                ...
                console.log('âœ… Returns fetched:', normalizedRows.length, 'records');
                console.log('Returns fetched:', normalizedRows.length, 'records');
                if (Array.isArray(normalizedRows)) return normalizedRows;
                //
        ...
        getSavedReports: async () => {
            const res = await api.get(`/reports/saved?_t=${new Date().getTime()}`);
            return res.data;
        },
