...
function getBatchKey(order) {
  ...
  const dateStr = order.dispatchDate
    ? format(new Date(order.dispatchDate), "yyyy-MM-dd")
    : "nodate";
...
function isItemReturned(item, returns = []) {
  ...
  const itemSerial = normalizeSerial(getItemSerial(item));
  if (!itemSerial || returns.length === 0) return false;

  const itemTime = new Date(item.dispatchDate || item.orderDate || item.createdAt || 0).getTime();

  return returns.some((r) => {
    const returnSerial = normalizeSerial(getReturnSerial(r));
    if (!returnSerial) return false;

    const isMatch = returnSerial === itemSerial || 
      (returnSerial.length > 3 && itemSerial.length > 3 && (returnSerial.includes(itemSerial) || itemSerial.includes(returnSerial)));

    if (isMatch) {
      const returnTime = new Date(r.returnDate || r.createdAt || 0).getTime();
      // If return happened after dispatch (with 24h buffer), it belongs to this order
      if (returnTime >= itemTime - 86400000) {
        return true;
      }
    }
...
function isItemReplaced(item) {
  ...
  const reason = item.reason || item.cancellationReason || item.holdReason || "";
  return reason.includes("Replaced returned serial:");
}

function getOldSerial(item) {
  const history = getReplacedSerialHistory();
  if (history[item.id]) return history[item.id].oldSerial;
...
function StatusTimeline({ currentStatus }) {
  ...
  return (
    <div className="bg-white rounded-lg border border-slate-0 p-[-12]">
...
export default function OrderTracking({ orders = [], onRefresh, models = [], serials = [], currentUser = null, isAdmin, isSupervisor }) {
  ...
  useEffect(() => { if (Array.isArray(models) && models.length > 0) setLocalModels(models); }, [models]);
  useEffect(() => { if (Array.isArray(serials) && serials.length > 0) setLocalSerials(serials); }, [serials]);
  useEffect(() => { setStatusFilter("All"); setSearchTerm(""); }, [activeTab]);

  useEffect(() => {
    let mounted = true;
    const loadDispatchData = async () => {
      const hasPropModels = Array.isArray(models) && models.length > 0;
      const hasPropSerials = Array.isArray(serials) && serials.length > 0;
      ...
      if ((hasPropModels || hasLocalModels) && (hasPropSerials || hasLocalSerials)) return;
      try {
  ...
  }, [models, serials, localModels, localSerials]);

  useEffect(() => {
    const loadReturns = async () => {
      try {
        const data = await printerService.getReturns();
        const returnsArray = Array.isArray(data) ? data : Array.isArray(data?.data) ? data.data : [];
  ...
  // ✅ Updated filteredBatches to handle hold & returned tabs
  const filteredBatches = useMemo(() => {
    return groupedBatches.map(batch => {
      const financials = calculateBatchFinancials(batch.items, returns);
      return { ...batch, financials };
    }).filter((batch) => {
      const isCompleted = batch.status === "Completed";
      const isCancelled = batch.status === "Order Cancelled";
      const isHold = isHoldStatus(batch.status);
      const hasReturns = batch.financials.returnedCount > 0 || batch.financials.replacedCount > 0;
      const hasActive = batch.financials.activeCount > 0;
      
      // Tab filtering
      if (activeTab === "active" && (!hasActive || isCompleted || isCancelled || isHold)) return false;
      if (activeTab === "returned" && !hasReturns) return false;
      if (activeTab === "hold" && !isHold) return false;
      ...
      if (activeTab === "returned") displayItems = batch.items.filter(i => isItemReturned(i, returns) || isItemReplaced(i));
  ...
  const stats = useMemo(() => {
    ...
    groupedBatches.forEach((b) => {
      ...
      const isCancelled = b.status === "Order Cancelled";
      const isHold = isHoldStatus(b.status);
      
      if (f.returnedCount > 0 || f.replacedCount > 0) returned++;
      if (f.activeCount > 0 && !isCompleted && !isCancelled && !isHold) active++;
      if (isHold) hold++;
  ...
  const handleSendForBilling = async (batch) => {
    ...
    try {
      await Promise.all(batch.items.map((item) => axios.put(`${API_BASE_URL}/api/orders/${item.id}/status`, { status: "Send for Billing" })));
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      ...
      await Promise.all(selectedBatch.items.map((item) => axios.put(`${API_BASE_URL}/api/orders/${item.id}/status`, payload)));
      showToast(
  ...
  const handleReplaceSerial = async (dispatchId, newSerialId, oldSerialValue) => {
    ...
    try {
      ...
      try {
        const history = JSON.parse(localStorage.getItem("replaced_serials") || "{}");
        history[dispatchId] = { oldSerial: oldSerialValue, newSerial: newSerialValue, timestamp: new Date().toISOString() };
        localStorage.setItem("replaced_serials", JSON.stringify(history));
      } catch(e) {}

      // 1. Update the serial number and attach tracking reason
      await printerService.updateDispatch(dispatchId, { 
        serialId: Number(newSerialId),
        serialNumberId: Number(newSerialId),
        cancellationReason: reasonText,
        reason: reasonText
      });
      
      // 2. Explicitly force status back to Active using the dedicated status endpoint
      await axios.put(`${API_BASE_URL}/api/orders/${dispatchId}/status`, {
      ...
      setLocalOrders(prev => prev.map(o => {
        if (o.id === dispatchId) {
          return { ...o, serialNumberId: newSerialId, serialId: newSerialId, serialValue: newSerialValue, serialNumber: newSerialValue, status: "Order Confirmed", logisticsStatus: "Packing in Process", reason: reasonText, cancellationReason: reasonText };
        }
        return o;
      }));
      ...
      setSelectedBatch(prev => {
        if(!prev) return prev;
        const newItems = prev.items.map(i => i.id === dispatchId ? { ...i, serialNumberId: newSerialId, serialId: newSerialId, serialValue: newSerialValue, serialNumber: newSerialValue, status: "Order Confirmed", logisticsStatus: "Packing in Process", reason: reasonText, cancellationReason: reasonText } : i);
        
        let newDisplayItems = newItems;
        if (activeTab === "active") newDisplayItems = newItems.filter(i => !isItemReturned(i, returns));
        if (activeTab === "returned") newDisplayItems = newItems.filter(i => isItemReturned(i, returns) || isItemReplaced(i));
        
        return { ...prev, items: newItems, displayItems: newDisplayItems };
      });
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredBatches.length > 0 ? (
                filteredBatches.map((batch, index) => {
                  ...
                  let displayStatus = resolveDisplayStatus(batch.status);
                  ...
                  if (!processingPhases.includes(batch.status) && !processingPhases.includes(displayStatus) && batch.logisticsStatus) {
                    displayStatus = batch.logisticsStatus;
                  }

                  if (financials.returnedCount > 0) {
                    displayStatus = financials.returnedCount === financials.totalCount ? "Returned" : "Partially Returned";
                  }
                  ...
                  const isCancelled = batch.status === "Order Cancelled";
                  const isOnHold = isHoldStatus(batch.status);
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 backdrop-blur-sm p-3 overflow-y-auto" onClick={closeModal}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl my-4" onClick={(e) => e.stopPropagation()}>
            ...
            {(() => {
              ...
              let modalDisplayStatus = resolveDisplayStatus(selectedBatch.status);
              ...
              if (!processingPhasesModal.includes(selectedBatch.status) && !processingPhasesModal.includes(modalDisplayStatus) && selectedBatch.logisticsStatus) {
                modalDisplayStatus = selectedBatch.logisticsStatus;
              }
              if (f.returnedCount > 0) {
                modalDisplayStatus = f.returnedCount === f.totalCount ? "Returned" : "Partially Returned";
              }

              const isCancelledOrder = selectedBatch.status === "Order Cancelled";
              const isOnHoldOrder = isHoldStatus(selectedBatch.status);
