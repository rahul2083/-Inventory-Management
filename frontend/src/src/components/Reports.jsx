...
export default function Reports({ isAdmin, isAccountant, isSupervisor }) {
  ...
  const processedData = useMemo(() => {
    ...
    Object.values(groups).forEach(batch => {
      batch.netProfit = batch.selling - batch.landing - batch.commission - batch.freight - batch.packing;

      if (damageStatuses.includes(batch.status) || damageStatuses.includes(batch.logisticsStatus)) {
        damageLoss += batch.landing;
      } else if (deliveredStatuses.includes(batch.status) || deliveredStatuses.includes(batch.logisticsStatus)) {
        revenue += batch.selling;
  ...
  const downloadCSV = () => {
    ...
    const summaryRows = [
      ...
      ["Net Profit (Delivered)", summary.netProfit.toLocaleString("en-IN")],
      ["Total Loss / Damage", summary.damageLoss.toLocaleString("en-IN")],
      [],
    ...
    const tableHeaders = [
      ...
      "Landing Price", "Selling Price", 
      "Commission", "Packing", "Freight", 
      "Net Profit"
    ];
        ...
        <div className="space-y-6 print:space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
            ...
            <div className={`bg-white p-4 rounded-2xl border shadow-sm relative overflow-hidden ${summary.netProfit >= 0 ? 'border-indigo-100' : 'border-red-100'}`}>
              ...
              <p className={`text-[10px] font-bold uppercase ${summary.netProfit >= 0 ? 'text-indigo-600' : 'text-red-500'}`}>Net Profit (Delivered)</p>
            ...
            <div className="bg-white p-4 rounded-2xl border border-red-100 shadow-sm relative overflow-hidden bg-red-50/20">
              ...
              <p className="text-[10px] font-bold text-red-500 uppercase">Total Loss / Damage</p>
