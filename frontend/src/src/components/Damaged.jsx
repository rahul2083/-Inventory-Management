...
export default function Damaged({ returns = [], isAdmin, isUser, onRefresh }) {
  ...
  const damagedItems = returns.filter((r) => {
    ...
    return (
      r.serialValue?.toLowerCase().includes(search) ||
      (r.serialNumberId?.modelId?.name || "").toLowerCase().includes(search) ||
      (r.firmName || "").toLowerCase().includes(search) ||
  ...
  const handleDelete = async (item) => {
    ...
    console.log("🔍 Full item object:", item);           // Debug
    console.log("🔍 item._id:", item._id);               // Debug
    console.log("🔍 item.id:", item.id);                  // Debug
    console.log("🔍 Resolved ID:", itemId);               // Debug
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/api/returns/${itemId}`, payload);
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      <span className="text-xs font-semibold text-slate-600">
                        {item.serialNumberId?.modelId?.name || "Unknown"}
                      </span>
                    </td>
